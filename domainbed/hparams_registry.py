...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('fresh_num', 1, lambda r: int(1))

        _hparam('w_ic', 0.01, lambda r: r.uniform(0.001,0.02))
        _hparam('gam2', 0.001, lambda r: r.uniform(0.0005,0.0015))
        _hparam('gam1', 0.001, lambda r: r.uniform(0.0005,0.0015))
        _hparam('N_p', 64, lambda r: r.choice([16,64]))
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('batch_size', 6, lambda r: int(r.uniform(6,12)))
    elif dataset == 'DomainNet':
        _hparam('batch_size', 6, lambda r: int(2**r.uniform(3, 5)))
    else:
        _hparam('batch_size', 6, lambda r: int(2**r.uniform(3, 5.5)))
